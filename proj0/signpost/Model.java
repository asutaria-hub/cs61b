...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int r = 0; r < _width; r++) {
            for (int c = 0; c < _height; c++) {
                int dir = arrowDirection(r, c);
                if (_solution[r][c] == 1) {
                    _board[r][c] = new Sq(r, c, 1, true, dir, 0);
                } else if (_solution[r][c] == last) {
                    _board[r][c] = new Sq(r, c, last, true, dir, 0);
                } else {
                    _board[r][c] = new Sq(r, c, 0, false, dir, -1);
                }


                _board[r][c]._successors = new PlaceList();
                _board[r][c]._predecessors = new PlaceList();
                _allSquares.add(_board[r][c]);
                _solnNumToPlace[_solution[r][c]] = pl(r, c);
            }
        }

        for (int i = 1; i < last + 1; i++) {
            if (_solnNumToPlace[i] == null) {
                throw badArgs("solution must contain all "
                        + "numbers from 1 - last");
            }
        }
        ...
        for (int r = 0; r < _width; r++) {
            for (int c = 0; c < _height; c++) {
                for (Place pl:_allSuccessors[r][c][_board[r][c]._dir]) {
                    _board[r][c]._successors.add(pl);
                    get(pl)._predecessors.add(pl(r, c));
                }
            }
        }
        _unconnected = last - 1;
    ...
    Model(Model model) {
        ...
        for (int r = 0; r < _width; r++) {
            for (int c = 0; c < _height; c++) {
                _board[r][c] = new Sq(model._board[r][c]);
                _allSquares.add(_board[r][c]);
            }
        }
        ...
        for (int r = 0; r < _width; r++) {
            for (int c = 0; c < _height; c++) {
                _board[r][c]._successor = model._board[r][c]._successor;
                _board[r][c]._predecessor = model._board[r][c]._predecessor;
                _board[r][c]._head = model._board[r][c]._head;
            }
        }
    }
    ...
    boolean autoconnect() {
        boolean auto = false;
        for (Sq c : this) {
            if (c._sequenceNum != 0 && c._successors != null) {
                for (Place check : c.successors()) {
                    Sq plSq = get(check);
                    if (plSq.sequenceNum() == c.sequenceNum() + 1) {
                        if (c.connectable(plSq)) {
                            if (c.connect(plSq)) {
                                c.connect(plSq);
                                auto = true;
                            }
    ...
    void solve() {
        for (int r = 0; r < _width; r++) {
            for (int c = 0; c < _height; c++) {
                _board[r][c]._sequenceNum = _solution[r][c];
            }
        }
        autoconnect();
        _unconnected = 0;
    ...
    private int arrowDirection(int x, int y) {
        ...
        for (int r = 0; r < _width; r++) {
            for (int c = 0; c < _height; c++) {
                if (_solution[r][c] == seq0 + 1) {
                    return Place.dirOf(x, y, r, c);
                }
            }
        }
        return 0;
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (this._dir != pl.dirOf(this.x, this.y, s1.x, s1.y)) {
                return false;
            }
        ...
        boolean connect(Sq s1) {
            ...
            Sq copy2 = this;
            while (copy2._successor != null) {
                copy2._successor._head = copy2._head;
                copy2 = copy2._successor;
            }
            return true;
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this.predecessor() == null && next.successor() == null) {
                    ...
                    int g = next.group(); this._group = -1; next._group = g;
                ...
                boolean test = true;
                for (Sq copy = this; copy != null; copy = copy._predecessor) {
                    if (copy._hasFixedNum) {
                        test = false;
                        break;
                    }
                }
                if (test) {
                    if (this._predecessor != null) {
                        int g = newGroup();
                        for (Sq copy = this; copy != null;
                             copy = copy._predecessor) {
                            copy._group = g; copy._sequenceNum = 0;
                        }
                    } else {
                ...
                boolean test2 = true;
                for (Sq copy = next; copy != null; copy = copy._successor) {
                    if (copy._hasFixedNum) {
                        test2 = false;
                        break;
                    }
                ...
                if (test2) {
                    if (next._successor != null) {
                        int g = newGroup();
                        for (Sq c = next; c != null; c = c._successor) {
                            c._group = g; c._sequenceNum = 0;
                        }
                    } else {
