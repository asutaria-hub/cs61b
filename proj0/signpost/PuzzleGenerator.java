...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        int pos = 0;
        Sq posSq = null;
        if (start.successors().size() == 1) {
            Sq end = model.get(start.successors().get(0).x,
                    start.successors().get(0).y);
            if (start.connectable(end)) {
                return end;
            }
        } else {
            for (int b = 0; b < start.successors().size(); b++) {
                Sq end = model.get(start.successors().get(b).x,
                        start.successors().get(b).y);
                if (start.connectable(end)) {
                    if (start.sequenceNum() + 1 == end.sequenceNum()
                            && start.sequenceNum() != 0) {
                        return end;
                    }
                    posSq = end;
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        int pos = 0;
        Sq posSq = null;
        PlaceList sol = end.predecessors();
        for (Place pred: sol) {
            if (model.get(pred).connectable(end)) {
                pos++;
                posSq = model.get(pred);
            }
        }
