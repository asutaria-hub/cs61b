...
public class RedBlackTree<T extends Comparable<T>> {
    ...
    RBTreeNode<T> buildRedBlackTree(BTree.Node<T> r) {
        ...
        if (r.getItemCount() == 3) {
            RBTreeNode temp = new RBTreeNode(true, r.getItemAt(1));
            temp.left = new RBTreeNode(false, r.getItemAt(0));
            temp.right =  new RBTreeNode(false, r.getItemAt(2));
            if(r.getChildrenCount() > 0) {
                temp.left.left = buildRedBlackTree(r.getChildAt(0));
                temp.left.right = buildRedBlackTree(r.getChildAt(1));
                temp.right.left = buildRedBlackTree(r.getChildAt(2));
                temp.right.right = buildRedBlackTree(r.getChildAt(3));
            }
           return temp;
        }
        else if (r.getItemCount() == 2) {
            RBTreeNode temp = new RBTreeNode(true, r.getItemAt(0));
            temp.right =  new RBTreeNode(false, r.getItemAt(1));
            if(r.getChildrenCount() > 0) {
                temp.left = buildRedBlackTree(r.getChildAt(0));
                temp.right.left = buildRedBlackTree(r.getChildAt(1));
                temp.right.right = buildRedBlackTree(r.getChildAt(2));
            }
           return temp;
        } else {
            RBTreeNode temp = new RBTreeNode(true, r.getItemAt(0));
    ...
    RBTreeNode<T> rotateRight(RBTreeNode<T> node) {
        RBTreeNode temp = new RBTreeNode<T>(false, node.item, node.left.right, node.right);
        node = new RBTreeNode<T>(node.isBlack, node.left.item, node.left.left, temp);
        return node;
    }
    ...
    RBTreeNode<T> rotateLeft(RBTreeNode<T> node) {
        RBTreeNode temp = new RBTreeNode<T>(false, node.item, node.left, node.right.left);
        node = new RBTreeNode<T>(node.isBlack, node.right.item, temp, node.right.right);
        return node;
    }
