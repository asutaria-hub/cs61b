...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] b) {
        if (b == null) {
            return null;
        }
        double[][] ab = new double[b.length][b[0].length];
        ...
        for (int row = 1; row < b.length; row++) {
            for (int col = 0; col < b[0].length; col++) {
                ...
                for (int i = -1; i <= 1; i++) {
                    if (get(ab, row - 1, col + i) < least) {
                        least = get(ab, row - 1, col + i);
                    }
                }
                ab[row][col] = least + b[row][col];
            }
        }
        return ab;
    }
